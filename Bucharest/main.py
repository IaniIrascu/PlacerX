...
df = pd.read_csv('department_enhanced.csv')

df2 = pd.read_csv('sectoare_cu_densitate.csv')
df3 = pd.read_csv('department_initial.csv')
...
for lat, long in zip(df2['Latitude'], df2['Longitude']):
    ...
    if not coordinates.empty:
        ...
        same_stores_within_radius = df[(df['Name'].str.contains('Primark')) & df.apply(within_radius, center=cluster_center, radius=2.5, axis=1)]
        ...
        if same_stores_within_radius.empty:
            ...
            midpoints.append(['Primark', address, cluster_center[0], cluster_center[1]])

midpoints_df = pd.DataFrame(midpoints, columns=['Name', 'Address', 'Latitude', 'Longitude'])
midpoints_df.to_csv("department_additions.csv", index=False)

df4 = pd.concat([df3, midpoints_df], ignore_index=True)
df4.to_csv("department_final.csv", index=False)